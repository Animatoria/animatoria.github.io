...
function editableAreaSwitch(e) {
  if (e.freshCardSwitch) {
    ...
    saveCardDataToLocalStorage(['cardNum_' + mainDate + '_' + mainTheme.value + '_' + e.cardNum], JSON.stringify(storedCard[e.cardNum]));
  }
  if (e.input.checked) {
...
function saveCardDataToLocalStorage(adress, item) {
  if (localStorage.getItem('') == thisSessionID && !readOnlyMode) {
    localStorage.setItem(adress, item);
  } else {
    k = 0;
    storedCard = [];
    card = [];
    ...
    readOnlyModeButton.onclick = function() {location.reload(true)};
    readOnlyModeButton.value = 'Make some changes';
    if (!readOnlyMode) alert('Read only, because new Rotation cards application opened at the  same time.');
    readOnlyMode = true;
    deleteAllCards();
    getStoredCards();
  }
