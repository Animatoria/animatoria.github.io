function rowCount(e) {
	...
	var wordsMap = makeWordsMap();  var lineSumm = 0;
	var finallyLines = 1;  for (var i in wordsMap) {
		lineSumm += wordsMap[i].letterLength;
		if (wordsMap[i].type == 'space') {
			lineSumm++;
		}
		if (lineSumm > flipperColumnWidth) {
			finallyLines++;
			if (wordsMap[i].letterLength == flipperColumnWidth) {
				lineSumm = 0;
			} else if (wordsMap[i].letterLength > flipperColumnWidth) {
				finallyLines += Math.floor((wordsMap[i].letterLength - 1)/flipperColumnWidth) - ((i == 0) || (wordsMap[i - 1].type == 'enter'));
				lineSumm = wordsMap[i].letterLength%flipperColumnWidth;
			} else {
				lineSumm = wordsMap[i].letterLength;
			}
			if (wordsMap[i].type == 'space') {
